...
static const GUI_POINT _aPointer[] = {			/*static const是只读全局变量声明*/
  { 0,  3},
  {85,  1},
  {90,  0},
  {85, -1},
...
typedef struct {
  GUI_AUTODEV_INFO AutoInfo;	             /*哪些内容需要绘制的信息*/
...
static void _DrawHiRes(void * p) {
  PARAM * pParam = (PARAM *)p;   /*强制类型转换，把指针P转换为结构体指针*/
  if (pParam->AutoInfo.DrawFixed) {
    GUI_ClearRect(60, 60, 159, 159); /*绘制固定的背景*/	  /*该函数功能是使用背景颜色填充一个矩形区域  达到清除效果*/
  }
  GUI_AA_FillPolygon(pParam->aPoints, 		 /*绘制移动的物体*/	  /*该函数的功能是绘制一个填充和抗锯的多边形*/
                     countof(_aPointer), 
                     65  * pParam->Factor, 	  /* ？*/
                     155 * pParam->Factor);
}
